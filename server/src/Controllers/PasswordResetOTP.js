...
const sendEmail = async (to, user) => {
  try {
   ...
   const mailOptions = {
    ...
    <div style="background-color: #FF0000; color: white; padding: 10px; text-align: center; border-top-left-radius: 8px; border-top-right-radius: 8px;">
        <h1>Your OTP Code</h1>
    </div>
    <div style="padding: 20px;">
        <p>Hello ${user.username},</p>
        <p>Thank you for using our service. Your One-Time Password (OTP) to proceed is:</p>
        <div style="font-size: 24px; font-weight: bold; color: #FF0000; text-align: center; margin: 20px 0; letter-spacing: 2px;">${otp}</div>
        <p>Please use this OTP within the next 5 minutes. If you did not request this code, please ignore this email.</p>
        <a href="${process.env.BACKEND_URL}/auth/resetPassword?email=${user.email}" style="display: inline-block; padding: 10px 20px; font-size: 16px; color: #ffffff; background-color: #FF0000; text-decoration: none; border-radius: 5px; margin: 20px 0;">Verify Now</a>
    </div>
...
const sendPasswordResetEmail = async(email) =>{
    try {
      console.log(email)
        ...
        return await sendEmail(email, user);
      } catch (error) {
